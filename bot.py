...
def simulate_new_hands(hand, discardSetList):
    """
    simulate_new_hands takes the list of current hand cards, and the list of discard sets. It iterates through that
    discard set list and finds the possible new hands that can be generated by calling the draw_combinations function.
    This function is called with the arguments of the length of the discard set (amount of cards to draw) and the
    available cards in draw pile (deck - hand)
    ...
    for discardSet in discardSetList:
        discardSet = set(discardSet)
        hand_temp = list(handSet - discardSet)
        for new_cards in itertools.combinations(newDeck.cards, len(discardSet)):
            new_hand = hand_temp + list(new_cards)
            key = frozenset(discardSet)
            if key not in newHandsPerDiscardSet:
                newHandsPerDiscardSet[key] = [new_hand]
            else:
                newHandsPerDiscardSet[key].append(new_hand)
...
def discard_rank_count_calculator(handSetDict, debug=False):
    ...
    discardRankCounts = {}
    for discardSet, handSets in handSetDict.items():
        discardRankCounts[discardSet] = {}
        # discarding 5 cards always have same combinations.
        if len(discardSet) == 5:
            discardRankCounts[discardSet] = {0: 775249, 1: 645300, 2: 71802, 3: 31502, 4: 4615, 5: 2986, 6: 2124,
                                             7: 344, 8: 17}
        else:
            with multiprocessing.Pool(multiprocessing.cpu_count()) as p:
                results = p.map(hand_rank, handSets)
            for hand in results:
                handValue = hand[0]
...
def calculate_relative_values(rankCounts, debug=False):
    rank_weights = {0: 0, 1: 1, 2: 5, 3: 10, 4: 25, 5: 50, 6: 100, 7: 1000, 8: 5000}
    relative_values = {}
...
def run_best_discard(hand, debug=False):
    ...
    if debug:
        ...
        max_cards_width = max(len(str(tuple(cards))) for cards in rank_counts.keys()) + 2
        rank_names = {
            0: 'High Card',
            1: 'One Pair',
            2: 'Two Pairs',
        ...
        for cards, ranks in probs.items():
            card_str = f"{str(tuple(cards))}".ljust(max_cards_width)
            row = [card_str] + [f"{ranks.get(rank, 0):>10.5f}" for rank in range(9)]  # right-align probabilities
            print("  ".join(row))
